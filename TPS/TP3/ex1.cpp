...
Result nearestPoints_BF(std::vector<Point> &vp) {
    Result res;
    bool first = true;
    //Brute force will loop all points and calculate their distances.

    for (Point p1: vp){
        for (Point p2: vp){
            if (p1 == p2) continue;
            if (first || res.dmin > p1.distance(p2)){
                first = false;
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    ...
    Result resM, resL, resR;
    resM.dmin = MAX_DOUBLE;
    sortByX(vp, 0, vp.size()-1);
    ...
    int mid = vp.size()/2;
    std::vector<Point> vpLeft(vp.begin(), vp.begin()+mid);
    std::vector<Point> vpRight(vp.begin()+ mid ,vp.end());
    std::vector<Point> vpMid(vp.begin()+ mid/2 ,vp.end()- mid /2);

    resL = nearestPoints_DC(vpLeft);
    resR = nearestPoints_DC(vpRight);
    resM = nearestPoints_DC(vpMid);

    resM = resM.dmin < resL.dmin ? resM : resL;
    resM = resM.dmin < resR.dmin ? resM : resR;


    std::vector<Point>vq(vp);
    sortByY(vq, 0, vp.size()-1);

    for (Point p1 : vq){
        for (Point p2: vq){
            if (p1 == p2) continue;
            if (abs(p1.x-p2.x)> resM.dmin)
                break;
            else if (resM.dmin > p1.distance(p2))
                resM = Result(p1.distance(p2),p1,p2);
        }
    }
